"""Specifies instructions for LENS.
"""
...
def task_to_instruction(task_name: str, is_query: bool = True) -> str:
    # Following BGE-EN-ICL, we add "." to the end of the instruction if it doesn't already have it
    if task_name in TASKNAME2INSTRUCTIONS:
        if isinstance(TASKNAME2INSTRUCTIONS[task_name], tuple):
            if is_query:
                return TASKNAME2INSTRUCTIONS[task_name][0] if "." == TASKNAME2INSTRUCTIONS[task_name][0][-1] else TASKNAME2INSTRUCTIONS[task_name][0] + "."
            else:
                return TASKNAME2INSTRUCTIONS[task_name][1]
        return TASKNAME2INSTRUCTIONS[task_name] if "." == TASKNAME2INSTRUCTIONS[task_name][-1] else TASKNAME2INSTRUCTIONS[task_name] + "."

    meta = mteb.get_task(task_name).metadata
    if DEFAULT_PROMPTS.get(meta.type, "")[-1] != ".":
