...
class BiEncoderModel(nn.Module):
    ...
    def pooling(self, vecs, pooling_mask):
        if self.pooling_method == 'cls-pooling':
        ...
        elif self.pooling_method == 'sum-pooling':
            return torch.sum(torch.log(1 + torch.relu(vecs)) * pooling_mask.unsqueeze(-1), dim=1)
        else:
    ...
    def encode(self, features):
        ...
        if not isinstance(features, list):
            ...
            for sub_features in features:
                pooling_mask = sub_features.pop('pooling_mask')
                psg_out = self.model(**sub_features, return_dict=True, output_hidden_states=False)
                p_reps = self.pooling(psg_out.logits, pooling_mask)
    ...
    def forward(self,
        ...
        if self.training:
            if messages[0] == 'normal':
                ...
                loss = self.compute_cross_entropy_loss(scores, target) # All samples in the batch except the positive one are treated as negatives
